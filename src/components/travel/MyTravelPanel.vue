<template>
  <div class="my-travel-overlay" @click.self="$emit('close')">
    <div class="my-travel-panel">
      ...
      <!-- 제목 -->
      <input v-model="tripTitle" class="title-input" placeholder="여정 이름을 입력하세요" />

      <!-- 빈 상태 -->
      <p v-if="store.plan.items.length === 0">추가된 장소가 없습니다.</p>

      <!-- 리스트 -->
      <draggable v-model="store.plan.items" item-key="placeId" handle=".drag-handle" animation="200" @end="onDragEnd">
        <template #item="{ element, index }">
          <div class="tour-item">
            <span class="drag-handle">☰</span>
            <span class="name">{{ index + 1 }}. {{ element.name }}</span>
            <button class="remove-btn" @click="remove(element.placeId)">✕</button>
          </div>
        </template>
      </draggable>
      ...
      <div class="panel-footer">
        <div class="register-wrapper" @mouseenter="showHint = true" @mouseleave="showHint = false">
          <button class="register-btn" :disabled="store.plan.items.length === 0" @click="registerTrip">
            여정 등록하기
          </button>

          <div v-if="showHint && store.plan.items.length === 0" class="hint-tooltip">
